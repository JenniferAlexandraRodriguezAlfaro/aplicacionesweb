...
$(document).ready(function () 
    {
        obtenerDatos(); //hacer la primer peticion con el id por default
    }
);
...
function mover(hacia)
{
    ...
    if( hacia == "R") {
        ...
        if(proximoValor <= 0){
             //NO
            alertify.alert('¡Atención!', 'Ya esta al Inicio.');
        }else {
            //SI
            indicePersonaje--;
            huboMovimiento = true;
        ...
        if(proximoValor >= MAXIMO_PERSONAJES) {
            //NO
            alertify.alert('¡Atención!', 'Ya esta al final');
        } else {
            //SI
            indicePersonaje++;
            huboMovimiento = true;
...
{
    $('#imgPersonaje').attr("src", "https://www.utn.edu.red/imagenes/rick_and_morty.png");
